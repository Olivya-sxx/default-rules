const defaultRules = [
  ...
  { id: 'lost', scene: '不知道做什么', action: '任选一个小动作', minimum: '只做 3 分钟或 2 页', starters: [{ name:'学英语', detail:'只学 3 分钟' }, { name:'看书', detail:'只读 2 页' }, { name:'创作', detail:'只做 5 分钟' }] },
...
const lostRule = defaultRules.find((rule) => rule.id === 'lost');
const oldLostRule = rules.find((rule) => rule.id === 'lost' && rule.action === '打开任务菜单');
if (oldLostRule) { rules = rules.map((rule) => rule === oldLostRule ? { ...lostRule } : rule); save(); }
...
function renderNow() {
  ...
  const starterChoices = rule.starters ? `<p class="starter-prompt">先点一个。只做它写的这么多。</p><div class="starter-list">${rule.starters.map((starter, index) => `<button class="starter ${selectedStarter?.name === starter.name ? 'is-selected' : ''}" data-starter="${index}"><strong>${starter.name}</strong><span>${starter.detail}</span></button>`).join('')}</div>` : '';
  const ready = !rule.starters || selectedStarter;
  $('#action-card').innerHTML = `<p class="section-label">默认动作</p><h2>${selectedStarter ? selectedStarter.name : rule.action}</h2><span class="minimum">最低标准：${selectedStarter ? selectedStarter.detail : rule.minimum}</span>${starterChoices}<div class="button-row"><button class="primary" id="done" ${ready ? '' : 'disabled'}>我做到了</button><button class="secondary" id="stuck">我卡住了</button></div>`;
  document.querySelectorAll('[data-rule]').forEach((button) => button.onclick = () => { activeRuleId = button.dataset.rule; selectedStarter = null; renderNow(); });
  document.querySelectorAll('[data-starter]').forEach((button) => button.onclick = () => { selectedStarter = rule.starters[Number(button.dataset.starter)]; renderNow(); });
  $('#done').onclick = () => addLog('完成');
  $('#stuck').onclick = () => { selectedReason = reasons[0]; renderReasons(); $('#failure-dialog').showModal(); };
}
function addLog(result, reason = '', note = '') { const rule = activeRule(); const starterNote = selectedStarter ? `选了：${selectedStarter.name}（${selectedStarter.detail}）` : ''; logs.unshift({ id:uid(), ruleId:rule.id, scene:rule.scene, result, reason, note:[starterNote, note].filter(Boolean).join('；'), time:Date.now() }); save(); renderReview(); if (result === '完成') $('#action-card').innerHTML += '<p class="quiet-note result-good">记下来了。到这里就够了。</p>'; }
...
$('#rule-form').addEventListener('submit', (event) => { if (event.submitter?.value === 'cancel') return; event.preventDefault(); const existing = rules.find((rule) => rule.id === editingId); const candidate = { id:editingId || uid(), scene:$('#rule-scene').value.trim(), action:$('#rule-action').value.trim(), minimum:$('#rule-minimum').value.trim(), ...(existing?.starters ? { starters:existing.starters } : {}) }; if (!candidate.scene || !candidate.action || !candidate.minimum) return; if (editingId) rules = rules.map((rule) => rule.id === editingId ? candidate : rule); else rules.push(candidate); activeRuleId = candidate.id; save(); renderNow(); renderRules(); $('#rule-dialog').close(); });
